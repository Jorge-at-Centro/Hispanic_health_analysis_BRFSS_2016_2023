...
This project uses data from he Behavioral Risk Factor Surveillance System (BRFSS) to measure the impact of recent changes on racial classification by the Office of Management and Budget (OMB) on racial distribution and health outcomes in Puerto Rico, New York, and Florida.
...
-   `script` is a folder containing two R markdown files with code:
    -   The first script reads in the data and performs basic data cleaning and wrangling instructions. Additionally, it generates a series of new variables for our analysis. It also saves the processed data as an RDS file to ease analysis later on.
...
Running `02_data_analysis.Rmd` will output a number of .xlxs files containing results from our total estimates and statistical analysis. Results are exported in this manner because they facilitated usability for the research team. Results from our analysis are already included in the outputs folder and can bused to visualize and analyze results.
