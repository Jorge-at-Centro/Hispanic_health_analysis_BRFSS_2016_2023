---
title: "Data Analysis BRFSS Data"
...
# Define target BRFSS health indicators for analysis. 
# These represent categorical responses for general health, asthma, diabetes, 
# coronary heart disease, cancer, and healthcare coverage status.
...
```{r Path config, message=FALSE,warning=FALSE}
...
We will load the cleaned data set we saved as .RDS in `01_data_wrangling.Rmd`.
...
We create helper functions that will be used to generate total population counts by race and ethnicity, total population counts and weighted means by race and ethnicity and health condition status, and perform statistical analysis.
...
```{r State Analysis, message=FALSE, warning=FALSE}
